...
#数据集分割：
rate = 0.7  # 【此处为本例核心代码，已省略，完整代码见教材P75；建议读者手工输入核心代码并进行调试，这样方能领会其含义】
train_len = int(len(Y)*rate)
...
for epoch in range(10):  #训练代数设置为10
    for x, y in train_loader: #使用上面打包的训练集进行训练
        pre_y = model3_1(x)
        loss = nn.CrossEntropyLoss()(pre_y,y)  # 【此处为本例核心代码，已省略，完整代码见教材P75；建议读者手工输入核心代码并进行调试，这样方能领会其含义】
